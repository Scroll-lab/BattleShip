public class Logic {
    ...
    public void shootPlay(int i, int j) {
        arrayBot[i][j]+=7;
    ...
    private void analyzeDead(int[][] array,int i,int j, int countShip)
    {
        ...
        for (int k = i-(countShip-1);k<=i+(countShip-1);k++) {
            for (int g=j-(countShip-1);g<=j+(countShip-1);g++) {
                if (testArrayPos(k,g)&&(array[k][g]==countShip+7)) countInjure++;
            }
        }
    ...
    private void testEndGame() 
    {
        ...
        for (int i=0;i<10;i++ ) {
            for (int j=0;j<10;j++ ) {
                if (arrayPlayer[i][j]>=15) countPlayer+= arrayPlayer[i][j];
                if (arrayBot[i][j]>=15) countBot += arrayBot[i][j];
            }
        }
        if (countPlayer== testNumber) endGame = 2;
        else if (countBot == testNumber) endGame = 1;
    }

    private void setInjure(int[][] mas,int i,int j)
    {
        if (testArrayPos(i,j)==true)
        {
